...
export default function BoostFollowersPage() {
  ...
  const totalCost = amount * COST_PER_FOLLOWER;

  const handleBoost = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!user) {
      toast.error("You must be logged in to boost followers");
      return;
    }
    ...
    try {
      const response = await fetch('/api/boost/followers', {
        ...
        body: JSON.stringify({ profileUrl, amount, userId: user.uid }),
      });
      ...
      toast.success("Followers boost initiated successfully!");
      router.push("/dashboard");
    } catch (error) {
      console.error("Error boosting followers:", error);
      toast.error("Failed to boost followers. Please try again later.");
    } finally {
        ...
        <div className="md:flex md:items-center md:justify-between mb-8">
          <div className="flex-1 min-w-0">
            <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate flex items-center">
              <FaUsers className="text-blue-500 mr-3" /> Boost Instagram Followers
            </h2>
          </div>
        </div>
          ...
          <div className="p-6">
            <form onSubmit={handleBoost} className="space-y-6">
              <div>
                ...
                <div className="mt-1">
                  <input
                    ...
                    className="block w-full rounded-md border border-gray-300 px-3 py-2 placeholder-gray-400 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-blue-500 sm:text-sm text-gray-900 bg-white"
                  />
                </div>
                <p className="mt-1 text-sm text-gray-500">
                  Enter the URL of the Instagram profile you want to boost
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Choose between 10 and 1,000 followers
                </p>
              </div>

              <div className="bg-gray-50 p-4 rounded-md">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-500">Cost per follower:</span>
                  <span className="text-sm font-medium text-gray-900 flex items-center">
                    <FaCoins className="text-yellow-500 mr-1" /> {COST_PER_FOLLOWER}
                  </span>
                </div>
