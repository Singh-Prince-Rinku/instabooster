...
export default function BoostLikesPage() {
  ...
  const totalCost = amount * COST_PER_LIKE;

  const handleBoost = async (e: React.FormEvent) => {
    e.preventDefault();

    if (!user) {
      toast.error("You must be logged in to boost likes");
      return;
    }
    ...
    try {
      // Modified to send the userId in the request for server-side processing
      ...
      toast.success("Likes boost initiated successfully!");
      router.push("/dashboard");
    } catch (error) {
      console.error("Error boosting likes:", error);
      toast.error("Failed to boost likes. Please try again later.");
    } finally {
        ...
        <div className="md:flex md:items-center md:justify-between mb-8">
          <div className="flex-1 min-w-0">
            <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate flex items-center">
              <FaHeart className="text-red-500 mr-3" /> Boost Instagram Likes
            </h2>
          </div>
        </div>
            ...
            <form onSubmit={handleBoost} className="space-y-6">
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Enter the full URL of the Instagram post you want to boost
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Choose between 10 and 1,000 likes
                </p>
              </div>

              <div className="bg-gray-50 p-4 rounded-md">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-gray-500">Cost per like:</span>
                  <span className="text-sm font-medium text-gray-900 flex items-center">
                    <FaCoins className="text-yellow-500 mr-1" /> {COST_PER_LIKE}
                  </span>
                </div>
