...
// Cost in coins per comment
const COST_PER_COMMENT = 3;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!amount || amount < 5 || amount > 100) {
      return NextResponse.json(
        { success: false, message: 'Amount must be between 5 and 100' },
        { status: 400 }
      );
    }
    ...
    // Calculate the cost
    const totalCost = Math.ceil(amount * COST_PER_COMMENT);
    ...
    try {
      ...
      const boost = await serverCreateBoost(userId, 'comments', postUrl, amount, totalCost);
      ...
      return NextResponse.json({
        success: true,
        message: 'Comments boost initiated successfully',
        data: {
          boost,
